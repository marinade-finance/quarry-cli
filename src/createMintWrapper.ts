...
export function installCreateMintWrapper(program: Command) {
  program.command("create-mint-wrapper")
...
export async function createMintWrapper({
  ...
  if (mint instanceof PublicKey) {
    ...
    console.log(`Create mint wrapper ${mintWrapper.toBase58()} for mint ${mint.toBase58()}`);
    tx = newWrapperTx;
  } else {
    const {
      mintWrapper,
    ...
    console.log(`Create mint ${mint.publicKey.toBase58()} with wrapper ${mintWrapper.toBase58()}`);
    tx = newWrapperAndMintTx;
  }
